...
public class ManyToOneAssociationTests {
    ...
    /* 설명. Association Mapping은 Entity 클래스 간의 관계를 매핑한느 것을 의미한다.(join을 위함)
        이를 통해 객체를 이용해 데이터베이스의 테이블 간의 관계를 활용해 한번에 객체로 조회할 수 있다.

   설명 1. 다중성에 의한 분류
        : 연관 관계가 있는 객체 관계에서 실제로 연관을 가지는 객체수에 따라 분류한다.
        - N:1(ManyToOne) 연관 관계
        - 1:N(OneToMany) 연관 관계
        - 1:1(OneToOne) 연관 관계
   ...
   설명 2. 방향에 따른 분류
        : 테이블의 연관 관계는 외래 키를 이용하여 양방향 연관 관계의 특징을 가ㅏ진다.
        반면, 객체는 참조에 의한 연관관계로 단방향이다.
        객체 간의 연관 관계를 양방향으로 만들 경우 반대쪽에서도 필드를 추가하여 참조를 보관하면 된다.
